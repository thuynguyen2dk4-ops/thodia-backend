...
export const updateStoreStatus = async (req: Request, res: Response) => {
    try {
        ...
        await pool.query('UPDATE user_stores SET status = $1 WHERE id = $2', [status, id]);
...
export const cancelAd = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        await pool.query("UPDATE sponsored_listings SET status = 'cancelled', end_date = NOW() WHERE id = $1", [id]);
        res.json({ success: true });
...
export const updateJobStatus = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        await pool.query('UPDATE recruitment_posts SET status = $1 WHERE id = $2', [status, id]);
        res.json({ success: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
};
...
export const deleteJob = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        await pool.query('DELETE FROM recruitment_posts WHERE id = $1', [id]);
        res.json({ success: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
};
...
export const approveClaim = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existingCheck = await client.query(
            `SELECT id FROM user_stores WHERE name_vi = $1 AND abs(lat - $2) < 0.0001 AND abs(lng - $3) < 0.0001`,
            [mapboxName, lat, lng]
        );
...
export const rejectClaim = async (req: Request, res: Response) => {
    try {
        ...
        await pool.query("UPDATE store_claims SET status = 'rejected' WHERE id = $1", [claimId]);
        res.json({ success: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
};