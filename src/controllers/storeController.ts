        ...
        if (id) {
            await pool.query(
                `UPDATE user_stores 
                 ...
                 WHERE id=$10`,
                [name_vi, address_vi, phone, category, latNum, lngNum, description_vi, isPremiumBool, image_url, id]
            );
        } else {
            const newStore = await pool.query(
                `INSERT INTO user_stores 
                 ...
                 RETURNING id`,
                [userId, name_vi, address_vi, phone, category, latNum, lngNum, description_vi, isPremiumBool, image_url]
            );
...
export const getApprovedStores = async (req: Request, res: Response) => {
    try {
        const result = await pool.query(
            "SELECT * FROM user_stores WHERE status = 'approved' ORDER BY is_premium DESC"
        );
        const formattedRows = result.rows.map(row => ({
            ...row,
            lat: parseFloat(row.lat),
...
export const getStoreMenu = async (req: Request, res: Response) => {
    try {
        ...
        try {
            ...
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS name VARCHAR(255)"); 
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS description_vi TEXT");
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
        } catch(e) {}
...
export const saveMenuItem = async (req: Request, res: Response) => {
    try {
        ...
        try {
            ...
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS name VARCHAR(255)");
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS description_vi TEXT");
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS image_url TEXT");
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS is_available BOOLEAN DEFAULT TRUE");
            await pool.query("ALTER TABLE store_menu_items ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
        } catch (e) { console.warn("⚠️ Auto-fix DB warning:", e); }
        ...
        if (id) {
            ...
            await pool.query(
                `INSERT INTO store_menu_items (store_id, name, name_vi, price, description_vi, image_url, is_available, created_at) 
...
export const deleteGalleryImage = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const result = await pool.query('DELETE FROM store_gallery WHERE id = $1', [imageId]);
