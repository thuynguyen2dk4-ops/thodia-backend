...
export const getReviews = async (req: Request, res: Response) => {
    try {
        ...
        try {
            // Fix bảng review
            await pool.query("ALTER TABLE store_reviews ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
            
            // Fix bảng profiles (Thêm full_name nếu thiếu để tránh lỗi JOIN)
            await pool.query("ALTER TABLE profiles ADD COLUMN IF NOT EXISTS full_name VARCHAR(255)");
...
export const addReview = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await pool.query(
            `INSERT INTO store_reviews (store_id, user_id, rating, comment, created_at) 
             VALUES ($1, $2, $3, $4, NOW())`,
        ...
        try {
            ...
            try {
                await pool.query("ALTER TABLE user_stores ADD COLUMN IF NOT EXISTS rating_avg DECIMAL(3, 2) DEFAULT 0");
                await pool.query("ALTER TABLE user_stores ADD COLUMN IF NOT EXISTS review_count INT DEFAULT 0");
                
                await pool.query(
                    `UPDATE user_stores 
                     SET rating_avg = $1, review_count = $2 
                     WHERE CAST(id AS TEXT) = $3`,
                    [Number(avg_rating || 0).toFixed(1), count || 0, String(storeId)]
                );
            } catch (updateErr) {
