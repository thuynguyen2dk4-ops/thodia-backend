...
exports.deleteVoucher = exports.updateVoucher = exports.createVoucher = exports.getStoreVouchers = exports.removeUserVoucher = exports.getUserVouchers = exports.saveUserVoucher = exports.getActiveVouchers = void 0;
...
const getActiveVouchers = async (req, res) => {
    ...
    try {
        ...
        try {
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
        }
        catch (e) { }
...
const saveUserVoucher = async (req, res) => {
    ...
    try {
        try {
            ...
            `);
            await db_1.pool.query("ALTER TABLE user_saved_vouchers ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
            await db_1.pool.query("ALTER TABLE profiles ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW()");
        }
        catch (e) { }
        ...
        const userCheck = await db_1.pool.query('SELECT id FROM profiles WHERE id = $1', [userId]);
        if (userCheck.rows.length === 0) {
            await db_1.pool.query(`INSERT INTO profiles (id, email, full_name, created_at, updated_at) 
                 VALUES ($1, $2, $3, NOW(), NOW())`, [userId, `${userId}@example.com`, 'New User']);
        }
        // Kiểm tra trùng
        const check = await db_1.pool.query('SELECT id FROM user_saved_vouchers WHERE user_id = $1 AND voucher_id = $2', [userId, voucherId]);
        if (check.rows.length > 0)
            return res.status(400).json({ message: "Đã lưu trước đó" });
        await db_1.pool.query('INSERT INTO user_saved_vouchers (user_id, voucher_id, created_at) VALUES ($1, $2, NOW())', [userId, voucherId]);
...
exports.saveUserVoucher = saveUserVoucher;
// 3. Lấy ví Voucher
...
const removeUserVoucher = async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query('DELETE FROM user_saved_vouchers WHERE id = $1', [id]);
...
exports.removeUserVoucher = removeUserVoucher;
// ==================================================================
...
const getStoreVouchers = async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT *, 
             ...
             ORDER BY created_at DESC`, [String(cleanStoreId)]);
...
const createVoucher = async (req, res) => {
    try {
        try {
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS title_vi VARCHAR(255)");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS discount_type VARCHAR(20) DEFAULT 'amount'");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS discount_value DECIMAL(12, 2)");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS min_order DECIMAL(12, 2) DEFAULT 0");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS max_uses INT DEFAULT 100");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS start_date TIMESTAMP DEFAULT NOW()");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE");
            await db_1.pool.query("ALTER TABLE store_vouchers ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
        }
        catch (dbErr) { }
        const { storeId, code, title_vi, discount_type, discount_value, min_order, max_uses, start_date, end_date, is_active } = req.body;
        ...
        await db_1.pool.query(`INSERT INTO store_vouchers 
...
const updateVoucher = async (req, res) => {
    try {
        ...
        await db_1.pool.query(`UPDATE store_vouchers 
             SET code=$1, title_vi=$2, discount_type=$3, discount_value=$4, 
                 min_order=$5, max_uses=$6, start_date=$7, end_date=$8, is_active=$9
             WHERE id=$10`, [code?.toUpperCase(), title_vi, discount_type, discount_value, min_order, max_uses, start_date, end_date, is_active, id]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
const deleteVoucher = async (req, res) => {
    try {
        const { id } = req.params;
        try {
            await db_1.pool.query('DELETE FROM user_saved_vouchers WHERE voucher_id = $1', [id]);
        }
        catch (e) { }
        await db_1.pool.query('DELETE FROM store_vouchers WHERE id = $1', [id]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
